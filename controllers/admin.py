...
def sections_list():
    ...
    return dict(course=course, sections=sections)
...
def sections_update():
    ...
    bulk_email_form = FORM(
        DIV(
            TEXTAREA(
                _name="emails_csv", requires=IS_NOT_EMPTY(), _class="form-control"
            ),
            _class="form-group",
        ),
        LABEL(
    ...
    if bulk_email_form.accepts(request, session):
        ...
        return redirect(
            "/%s/admin/sections_update?id=%d" % (request.application, section.id)
        )
    elif bulk_email_form.errors:
        response.flash = "Error Processing Request"
    return dict(
        section=section, users=section.get_users(), bulk_email_form=bulk_email_form
    )
...
def admin():
    ...
    sectionsQuery = db(
        db.sections.course_id == courseid
    ).select()  # Querying to find all sections for that given course_id found above
    sectionsList = []
...
def add__or_update_assignment_question():
    ...
    if question_type in ("activecode", "actex"):
        if auto_grade not in ("unittest", "interact"):
            autograde = "manual"
            which_to_grade = ""
    try:

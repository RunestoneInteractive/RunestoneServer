...
conn = psycopg2.connect(
    host="localhost",
    user="bnmnetp_courselib",
    password="al3xandria",
    database="bnmnetp_courselib",
)

curs = conn.cursor()
curs.execute("set session statement_timeout to 0")
curs.execute(
    """select course_id, count(*), max(timestamp) from useinfo group by course_id having max(timestamp) < now() - interval '365 days' and count(*) < 100"""
)

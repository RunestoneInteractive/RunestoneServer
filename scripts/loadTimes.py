...
timepat = re.compile(r".*/runestone/(ajax|proxy)/(\w+)(\s|\?).*\s(\d+)\s+msecs.*")
pagepat = re.compile(r".*GET\s+/.*/(\w+)\.(html|js|css|png|jpg).*\s(\d+)\s+msecs.*")
datepat = re.compile(
    r".*\[((Mon|Tue|Wed|Thu|Fri|Sat|Sun) (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec).*\d\d\d\d)\].*"
)
print(dday)
kills = {}
runtimes = {}
...
for fname in sys.argv[ix:]:
    ...
    for line in logfile:
        ...
        if (currentday and dday == currentday.date()) or dday == None:
            ...
            if gt:
                if gt.group(2) not in runtimes:
                    runtimes[gt.group(2)] = []
                runtimes[gt.group(2)].append(int(gt.group(4)))
...
for k in sorted(runtimes, key=lambda x: sum(runtimes[x]) / len(runtimes[x])):
    print(
        "%20s\t%7d\t%6.3f\t%7d"
        % (k, len(runtimes[k]), sum(runtimes[k]) / len(runtimes[k]), max(runtimes[k]))
    )
...
for k in sorted(pagetimes, key=lambda x: sum(pagetimes[x]) / len(pagetimes[x])):
    print(
        ...
        % (
            k,
            len(pagetimes[k]),
            sum(pagetimes[k]) / len(pagetimes[k]),
            max(pagetimes[k]),
        )
    )

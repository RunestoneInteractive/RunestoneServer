...
def splitFiles(filename, directory):
    ...
    for idx, val in enumerate(lines):
        if val.startswith("----") or val.startswith(
            "~~~~~"
        ):  # a new subsection is identified by ----- or ~~~~~. Enter if when a new subsection starts
            if (
                not startCommentsTraversed
            ):  # enters this if only for the first section, to handle the chapter comments
                startComments = [
                ...
                for sub_idx, sub_val in enumerate(lines[0 : (idx - 1)]):
                    if sub_val.startswith(
                        "===="
                    ):  # handle the introduction section of the chapter
                        topicName = removeChars(
                            lines[sub_idx - 1].strip(), '\/:*?"-+`,!<>|'
                        )
                        newFile = (
                            newFilePath
                            + "\\intro-"
                ...
                ocf.writelines(
                    "   " + directory + "/" + topicName.replace(" ", "") + ".rst\n"
                )  # write new filename to file, for TOC
                o = open(newFile, "w")

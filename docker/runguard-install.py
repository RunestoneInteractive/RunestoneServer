...
def get_config(param_name, install_dir):
    ...
    return dict(
        jobe_max_users=10, clean_up_path="/tmp;/var/tmp;/var/crash;/run/lock;/var/lock"
    )[param_name]
...
def get_webserver():
    ...
    try:
        ps_lines = (
            subprocess.check_output(ps_cmd, shell=True).decode("utf8").split("\n")
        )
    except subprocess.CalledProcessError:
        raise Exception(
            "ps command to find web-server user id failed. Is the web server running?"
        )
    names = {ps_line.split(" ")[0] for ps_line in ps_lines}
    candidates = names.intersection(set(["apache", "www-data"]))
    if len(candidates) != 1:
        raise Exception(
            "Couldn't determine web-server user id. Is the web server running?"
        )
...
def check_php_version():
    ...
    if (
        subprocess.call(
            'php -r \'exit(version_compare(PHP_VERSION, "5.5.0", "lt"));\'', shell=True
        )
        != 0
    ):
...
def make_sudoers(install_dir, webserver_user, num_jobe_users):
    ...
    for i in range(num_jobe_users):
        ...
        for directory in get_config("clean_up_path", install_dir).split(";"):
            commands.append(
                "/usr/bin/find {}/ -user jobe{:02d} -delete".format(directory, i)
            )
...
def make_user(username, comment, make_home_dir=False, group="jobe"):
    ...
    try:
        ...
        if group is None:
            ...
            group_opt = " -g " + group
        do_command(
            'useradd -r {} -s "/bin/false"{} -c "{}" {}'.format(
                opt, group_opt, comment, username
            )
        )
...
def make_directory(dirpath, owner, group, permissions=771):
    ...
    if not os.path.exists(dirpath):
        os.makedirs(dirpath)
    do_command(
        "chown {0}:{1} {2}; chmod {3} {2}".format(owner, group, dirpath, permissions)
    )
...
def main():
    ...
    if subprocess.check_output("whoami", shell=True) != b"root\n":
        ...
        try:
            ...
            make_user(
                "jobe", "Jobe user. Provides home for runs and files.", True, None
            )

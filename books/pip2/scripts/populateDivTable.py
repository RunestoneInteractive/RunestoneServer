...
engine = create_engine('postgres://presnick@localhost/mydb' ) # 'postgresql+psycopg2://bmiller@localhost/bmiller'
    ...
    for line in fh:
        ...
        if mo:
            ...
            if divt == 'actex' and divid in odd_ex_list:
                divt = 'actex_answered'
            div = Divs(course_name=course_name, chapter=chapter, subchapter=subchapter, div_type=divt, div_id=divid)
            session.add(div)

    session.commit()

# clear existing rows for this book
session.query(Divs).filter(Divs.course_name==course_name).delete()

...
// style unit test results (which are generated by Skulpt)
...
$(document).on("DOMNodeInserted", '.unittest-results', function (ev) {
    ...
    var config = { attributes: true, attributeFilter: ['style'], childList: true, characterData: true };
    // pass in the target node, as well as the observer options
    observer.observe(unittest_results_target, config);
    ...
    /*if (div.html()) {
        ...
        var p = $(".unittest-results p");
        var text = p.text().split(' ');
        for (var i=0; i<text.length; i++) {
            if (text[i].indexOf('%') >= 0) {
                var pct=text[i];
                var html = '<div class="progress">';
                html += '       <div class="progress-bar progress-bar-success" style="width:'+pct+';"></div>';
                html += '   </div>';
                var bar = $(html);
